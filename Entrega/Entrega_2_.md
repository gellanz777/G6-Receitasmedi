...
> **Observação:** esta versão não depende de arquivos `.png`; todos os diagramas foram convertidos para blocos `mermaid`, que renderizam direto no Markdown em plataformas compatíveis.
...
O diagrama em Mermaid abaixo consolida os casos de uso recebidos no arquivo
...
```mermaid
flowchart LR
    Medico["Médico autenticado"]
    Auditor["Auditor / Administrador"]
    Farmacia["Farmácia G8"]
    G5["Sistema G5\nProntuário"]

    subgraph G6["Sistema G6 — Receitas Médicas"]
        UC01(["UC01\nEmitir Receita Médica"])
        UC02(["UC02\nSubstituir Receita Médica"])
        UC03(["UC03\nValidar Receita na Farmácia"])
        UC04(["UC04\nAutenticar Usuário"])
        UC05(["Consultar Histórico de Receitas"])
    end
...
O diagrama de classes em Mermaid abaixo representa os principais elementos de
...
classDiagram
    ...
    UsuarioController --> validartoken : usa
    validartoken --> Usuario : consulta token
    ReceitaController --> Receita : controla
    ReceitaController --> ReceitaItem : controla itens
    Receita "1" *-- "1..*" ReceitaItem : possui
    ReceitaController --> ProntuarioG5 : valida prontuário
    FarmaciaG8 --> ReceitaController : consome API
    Receita --> PostgreSQL : persiste
    ReceitaItem --> PostgreSQL : persiste
    Usuario --> PostgreSQL : persiste
```
...
O modelo de dados em Mermaid, baseado nas tabelas receita, receita_item e usuario, foi
...
```mermaid
erDiagram
    USUARIO ||--o{ RECEITA : emite
    RECEITA ||--|{ RECEITA_ITEM : possui
    RECEITA ||--o| RECEITA : substitui
...
Os diagramas de sequência foram convertidos para Mermaid, eliminando a dependência de arquivos PNG. Eles representam fluxos com comunicação entre módulos, conforme
...
sequenceDiagram
    ...
    participant Controller as ReceitaController
    participant G5 as Prontuário G5
    participant Receita
    participant Item as ReceitaItem

    Medico->>Frontend: Preenche receita
    Frontend->>Controller: POST /receita
    Controller->>Controller: Validar campos e token
    Controller->>G5: Validar prontuário
    G5-->>Controller: Prontuário válido
    Controller->>Receita: Criar receita
    Receita-->>Controller: Receita criada
    Controller->>Item: Criar itens
    Item-->>Controller: Itens salvos
    Controller-->>Frontend: Retorna receita emitida
    Frontend-->>Medico: Exibe confirmação
```
...
```mermaid
sequenceDiagram
    participant Farmacia as Farmácia G8
    participant Controller as ReceitaController
    participant Receita
    participant Item as ReceitaItem

    Farmacia->>Controller: Enviar UUID
    Controller->>Receita: Buscar receita por código
    Receita-->>Controller: Receita encontrada
    Controller->>Controller: Verificar status
    Controller->>Item: Buscar medicamentos
    Item-->>Controller: Itens da receita
    Controller-->>Farmacia: Receita válida
    Farmacia->>Controller: Dispensar
    Controller->>Receita: Atualizar status para dispensada
    Receita-->>Controller: Status atualizado
    Controller-->>Farmacia: Status dispensada
```
...
A arquitetura em Mermaid mantém a separação entre interface, API, middleware,
...
flowchart LR
    ...
    G8["Farmácia G8\nvalida e dispensa receita"]
    G5["G5 Prontuário\nGET /prontuario/paciente/:id"]

    subgraph G6["API G6 — Receitas Médicas | Porta 3006"]
        API["Node.js + Express\nRotas REST JSON"]
        Token["Middleware validartoken\nheader token UUID"]
        UsuarioCtrl["UsuarioController\nlogin e token"]
        ReceitaCtrl["ReceitaController\nemissão, consulta, substituição, validação e dispensação"]
        Models["Models Sequelize\nUsuario, Receita, ReceitaItem"]
    end

    DB[("PostgreSQL\nusuario, receita, receita_item")]

    Frontend -->|HTTP JSON + token| API
    G8 -->|GET validar / POST dispensar| API
    API --> Token
    API --> UsuarioCtrl
    API --> ReceitaCtrl
    ReceitaCtrl -->|consulta prontuário| G5
    ReceitaCtrl --> Models
    UsuarioCtrl --> Models
    Models --> DB
```
